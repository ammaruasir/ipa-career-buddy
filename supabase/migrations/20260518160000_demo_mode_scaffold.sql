-- Demo Mode scaffold.
-- Adds `is_demo` flag to user-content tables touched by the demo tour and
-- installs symmetric RLS so demo accounts ONLY see demo rows and regular
-- users NEVER see demo rows. Auth user creation is handled by
-- scripts/seed-demo-data.ts via the admin API (migrations can't reliably
-- touch auth.users).
